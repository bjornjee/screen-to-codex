...
5. Contract: each installer pins one release tag and installs one app into `~/Applications` (or an explicit directory); updates back up the existing matching app and restore it on replacement failure, using the same destination lock as source builds.
...
8. Failure: unsupported hosts, network failures, invalid checksums/signatures, symlinks, and unrelated destination apps fail closed; no launch or credential access occurs during installation. Relative paths and Rosetta on Apple Silicon are accepted.
